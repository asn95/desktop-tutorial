    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative w-full max-w-md rounded-md border border-gray-200 bg-white p-6 shadow-xl dark:border-slate-600 dark:bg-slate-800">
        <h2 className="text-lg font-bold text-gray-900 dark:text-white">
          {t("Mulai periode")} {periodLabel}?
        </h2>
        <p className="mt-2 text-sm leading-relaxed text-gray-600 dark:text-slate-300">
          <span className="font-semibold">{rowCount} {t("target")}</span>{" "}
          {t("akan diunggah sebagai batch")}{" "}
          <span className="font-semibold">{periodLabel}</span>.{" "}
          {t("Data periode sebelumnya tetap tersimpan dan bisa dibuka kapan saja lewat pemilih periode.")}
        </p>
        ...
        <div className="mt-6 flex justify-end gap-2">
          ...
          >
            {isUploading ? t("Mengunggah…") : t("Ya, Unggah Sekarang")}
          </button>
        </div>
      </div>
    </div>
